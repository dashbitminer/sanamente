...
<div class="py-4 mt-2">
    <x-popover>
        <x-popover.button class="flex items-center gap-2 py-2 pl-3 pr-2 text-sm text-gray-600 border rounded-lg">
            <div>
        ...
        <x-popover.panel class="z-10 border border-gray-100 shadow-xl" position="bottom-end">
            <div class="flex flex-col divide-y divide-gray-100 min-w-64">
                @foreach (\App\Livewire\FGSM\Index\Range::cases() as $range)
                    @if ($range === \App\Livewire\FGSM\Index\Range::Custom)

                        <div x-data="{ showCustomRangePanel: $wire.filters.range === '{{ \App\Livewire\FGSM\Index\Range::Custom }}' ? true : false }">
                            ...
                            <form
                                ...
                                wire:submit="$set('filters.range', '{{ \App\Livewire\FGSM\Index\Range::Custom }}')"
