<div class="flex flex-col col-span-5 gap-2 sm:flex-row sm:justify-end">
    <div class="flex flex-row-reverse justify-end gap-2 sm:justify-start sm:flex-row" x-show="$wire.selectedRecordIds.length > 0" x-cloak>
        ...
        @if(auth()->user()->can('Importar inscripción formaciones SM a GWDATA'))
            <form wire:submit="procesarSelected"
                wire:confirm="¿Esta segura(o) que desea importar los registros seleccionados?"
            >
                ...
                >
                    <x-icon.receipt-refund wire:loading.remove wire:target="procesarSelected" />
    ...
    <div class="hidden sm:flex">
        <form wire:submit="export">
            <button type="submit" class="flex items-center gap-2 rounded-lg border px-3 py-1.5 bg-green-600 font-medium text-sm text-white hover:bg-green-500 border-green-600 hover:border-green-500"">
