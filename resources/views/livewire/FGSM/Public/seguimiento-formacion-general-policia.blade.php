<div class="px-4 sm:px-0">
    ...
    <h1 class="mt-10 mb-10 text-4xl font-bold text-center">Formulario de inscripción y asistencia PNC: <br/>Seguimiento de formación general</h1>
        ...
        <div class="space-y-12">
            ...
            <div class="pt-12 pb-12 border-b border-gray-900/10">
                    ...
                    <div class="px-4 py-3">
                        <div class="flex gap-6">
                            <x-input-label class="flex items-center h-12 gap-2 sm:text-lg">
                                <x-forms.input-radio type="radio" wire:model.live="form.participacion_policia" id="form.participacion" wire:key="participacion1"
                                name="form.participacion" type="radio" value="1" class="h-12 sm:text-lg"/>
                                Si
                            </x-input-label>
                            <x-input-label class="flex items-center h-12 gap-2 sm:text-lg">
                                <x-forms.input-radio type="radio" wire:model.live="form.participacion_policia" id="form.participacion" wire:key="participacion2"
                                name="form.participacion" type="radio" value="2" class="h-12 sm:text-lg"/>
                                No
                            </x-input-label>
                        </div>
                        <x-input-error :messages="$errors->get('form.participacion_policia')" class="mt-2" aria-live="assertive"/>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 mt-10 gap-x-6 gap-y-8 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-3">
                        <x-input-label class="sm:text-lg" for="form.documento_identidad_policia">{{ __('Escribe los últimos 4 dígitos de tu documento de identidad (DUI)') }}
                            <x-required-label />
                        </x-input-label>
                        <div class="mt-2">
                            <x-text-input wire:model.blur="form.documento_identidad_policia" id="form.documento_identidad_policia"
                                name="form.documento_identidad_policia" type="text"
                                placeholder="0000"
                                x-mask="9999"
                                {{--
                                ...
                                $errors->has('form.documento_identidad_policia')
                                ])
                                />
                                <x-input-error :messages="$errors->get('form.documento_identidad_policia')" class="mt-2" />
                        </div>
                    </div>

                    <div class="sm:col-span-3">
                        <x-input-label class="sm:text-lg" for="form.nombres">{{ __('Escribe tu nombre o indicatvo ficticio (Ejemplo: Oscar, Ramiro, Adolfo, etc.)') }}
                            <x-required-label />
                        </x-input-label>
                        <div class="mt-2">
                    ...
                    <div class="sm:col-span-3">
                        <x-input-label class="sm:text-lg" for="form.apellidos">{{ __('Escribe tus apellidos o indicativo ficticios (Ejemplo: Flores, Pérez, González, etc.)') }}
                            <x-required-label />
                        </x-input-label>
                        <div class="mt-2">
                            ...
                            <select name="perfiles" id="perfilSelected" wire:model.live="form.perfilSelected"
                             x-on:change="const selectedOption = event.target.options[event.target.selectedIndex];
                                ...
                                @foreach($perfiles as $perfil)
                                    @if ($perfil->id == $form->tipoPerfilPolicia  )
                                        <option value="{{ $perfil->id }}" data-perfil-options="{{ $perfil->pivotid }}" selected>
                                            {{ $perfil->nombre }}
                                        </option>
                                    @endif
                                @endforeach
                    ...
                    <div class="col-span-full"
                        x-show="$wire.form.perfilPoliciaSelected == {{ $form->tipoPerfilPoliciaNacional }} ">
...
<x-notifications.modal-confirmacion-finalizar>
    ...
    <div class="mt-2">
        <p class="text-lg text-gray-500">Gracias {{ $form->nombres.' '.$form->apellidos }} por completar el proceso, su código de confirmación es: <span class="font-semibold">
            {{ $form->numero_confirmacion }}</span></p>
    </div>
</x-notifications.modal-confirmacion-finalizar>
    ...
    document.addEventListener('livewire:navigated', function () {
      ...
      //  new Choices($wire.$el.querySelector('[data-intervencionistas]'), { shouldSort: false });
        ...
        Livewire.on('refresh-choices', subcategories => {
            ...
            subcategoryElement.innerHTML = '<option value="">Seleccione una escuela</option>';
